...
| Spinal curve model — the deformity envelope the fit must represent without flagging it. How much scoliotic / kyphotic curvature is normal anatomy the model must follow, versus deviation it must report; and the accepted false-negative cost of a stiffer fit | 119, 120, 121, 123, 125 | ⏳ Awaiting | Reach is item-level on purpose, and the Blocks cell is the only statement of it — the decision affects one thread while the queue keeps producing other work ([`.aide/conventions.md`](../../.aide/conventions.md) §Human gates, first row). Held: 119 (implements the formulation) and 120, 121, 123, 125 downstream of it. Released: **118**, the deliberation that produces the evidence this gate is decided on — gating it would block the work that feeds the gate; **122** (signed curvature, a defect in how tangents are reduced to a scalar, independent of the fit); **124** (the catalogue's observed-range column). Not derivable from the corpus: the synthetic fixtures carry one fixed 6 mm curve amplitude and no pathology, so the envelope is a clinical judgement. Real VerSe19 GT is available locally to evidence the proposal. Re-scoped 2026-08-27 from `stage 28`, which held all eight items including 118 — see [`insights.md`](insights.md). |
