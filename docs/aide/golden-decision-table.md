...
| tests/corpus/119_pre_119_digests.json | Two sha256 digests captured while item 119's branch still held the pre-119 tree — the bytes of `src/segfacet/pipeline.py`, and the sorted set of leaf `path` values in the committed `docs/aide/feature_catalogue.generated.json` — each compared against the same quantity recomputed at test time, so item 119's fit change is proved to have touched neither. Scoped to item 119 by construction: these are blast-radius fences, not permanent invariants, and stage 28's item 120 edits `pipeline.py` and so ends the first one's life. | tests/test_119_curve_formulation.py::test_ac22_pipeline_is_byte_identical_to_pre_119, tests/test_119_curve_formulation.py::test_ac27_catalogue_leaf_path_set_unchanged_from_pre_119 | n/a | keep | — |
