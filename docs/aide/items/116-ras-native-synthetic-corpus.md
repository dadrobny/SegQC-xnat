...
- **`reference_default.json` regenerated via its own module entry point
  (AC10, round-2 repair).** Round 1 missed this file because it lives outside
  `tests/corpus/**`; validation round 1 caught it and extended the authorised
  paths (commit `8444417`). Regenerated with
  `python -m segfacet.reference.artifact` (`build_and_write_default`, which
  runs `build_clean_spine` + `paint_clean_scan(seed=0)` over the fixed
  `_DEFAULT_COHORT_RECIPE` and writes through `write_artifact`) — the module's
  own documented regeneration path, not a hand edit. Confirmed
  byte-identical across two successive regenerations (505 lines changed
  from the pre-migration committed copy; zero lines differ between the two
  post-migration runs). **This does not change any runtime default:**
  `reference_verse_v1.json` (80 real VerSe19 subjects) has been the CLI's
  default reference since item 090, and `reference_default.json` is kept
  only as a fixed synthetic fallback / regression fixture, built from
  synthetic data that never ships as production behaviour. Regenerating it
  changes a test fixture's bytes (new voxel geometry under the RAS-native
  stacking axis), not what `segfacet run`/`evaluate` do against real data.

- **AC10 audit — committed artifacts derived from the synthetic generator.**
  Beyond `tests/corpus/**` (already regenerated by the prior commit) and
  `reference_default.json` (regenerated above), swept every production
  consumer of `segfacet.synth` (`grep` over `src/segfacet/**`) for a second
  committed, generator-derived artifact:
  - `catalogue.py` calls `build_clean_spine` to drive
    `docs/aide/feature_catalogue.generated.{json,md}` (item 103). Regenerated
    both into a scratch directory via `python -m segfacet.catalogue` and
    diffed against the committed copies: **byte-identical, not stale**. The
    catalogue records leaf *paths* and static rule/mode metadata, not
    computed numeric feature values, so it is insensitive to the voxel
    geometry this migration changed. Left untouched (also not in this
    item's Authorised paths, so it would have been a stop-and-report if it
    had drifted).
  - `benchmark.py` calls into `synth.corpus`/`synth.regression` but its own
    docstring is explicit: "Regenerate, do not commit" — no report is ever
    committed. Cleared, nothing to do.
  - `eval/severity_ladder.py` calls `build_clean_spine` at runtime (ladder
    construction for the Stage-16 monotonicity harness) but produces no
    committed artifact of its own. Cleared.
  - `scripts/refresh_reference.py` (item 083's refresh wrapper) also calls
    `build_clean_spine`, but its docstring guarantees it writes only under a
    caller-chosen `--out` directory, "never touching the committed package
    copy." Cleared.
  - `src/segfacet/reference/reference_verse_v1.json` is built from real
    VerSe19 data (item 090), not the synthetic generator. Cleared, out of
    scope for this migration by construction.
  - `tests/golden/016_features_report.json` and
    `tests/golden/022_stage3_report.json` were checked because they sit
    outside `tests/corpus/**`, but their owning tests import fixtures from
    `tests/synthetic.py` (hand-built happy-path fixtures), never
    `segfacet.synth`/`build_clean_spine`. Cleared — unrelated generator, no
    drift possible from this migration.
  - The three JSON Schemas under `src/segfacet/**` (`report_schema_v0.json`,
    `eval/eval_report_schema_v0.json`, `eval/per_mode_comparison_schema_v0.json`)
    are hand-authored schemas with no generator dependency. Cleared.

  No other stale, committed, generator-derived artifact was found.
