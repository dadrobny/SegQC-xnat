...
- AC15: an unmapped-rule-only, non-anchor entry gets an honest empty mode
        list with ``mode_evidence == ("rule_unmapped",)``.
...
def test_ac15_unmapped_rule_only_entry_is_honestly_unmapped(
    full_catalogue, feature_docs_module
):
    unmapped_rules = {"bounds", "intensity", "reference_delta", "intensity_reference_delta"}
    anchor_paths = {
        p for paths in feature_docs_module.MODE_ANCHOR_PATHS.values() for p in paths
    }
    candidates = [
        ...
        and set(e.consuming_rules) <= unmapped_rules
        and e.path not in anchor_paths
    ]
    assert candidates, "expected at least one unmapped-rule-only, non-anchor entry"
