"""Empty and near-empty segmentation detection for ``segfacet`` (item 007).
...
thresholds from :class:`~segfacet.config.HeuristicConfig`.
...
1. **No runtime imports beyond stdlib, NumPy, and NiBabel**: ``scipy``,
   ``skimage``, and ``segfacet.verdict`` are deliberately excluded; this module
...
def check_empty(
    ...
    config:
        :class:`~segfacet.config.HeuristicConfig` supplying the threshold
