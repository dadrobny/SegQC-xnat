"""segfacet.reference.delta -- per-vertebra delta-to-reference metrics
(Stage 6, item 046).

Scores a case's already-extracted per-label features (the ``features_block``
produced by :func:`segfacet.pipeline.extract_feature_record`) against a loaded
045 :class:`~segfacet.reference.schema.ReferenceDistribution`, computing for each
...
engine) and does **not** touch ``segfacet.heuristics``/``segfacet.config``.
