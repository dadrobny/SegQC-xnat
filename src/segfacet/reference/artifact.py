"""segfacet.reference.artifact — versioned reference-data artifact + builder
(Stage 6, item 045).

Chains item 044's cohort ingestion (``segfacet.reference.ingest.ingest_cohort``)
and item 043's aggregation core (``segfacet.reference.aggregate_reference``)
...
* :func:`build_reference` — ``cohort_dir`` -> :class:`~segfacet.reference.schema.ReferenceDistribution`,
  stamping a deterministic :class:`~segfacet.reference.schema.Provenance`
...
* :func:`build_default_cohort` / :func:`build_and_write_default` — the fixed,
  ...
  :func:`segfacet.synth.clean_gt.build_clean_spine` for the label maps and item
  058's :func:`segfacet.synth.intensity.paint_clean_scan` for a co-registered
  painted scan per subject, both seeded/no-wall-clock) that produces the
  bundled default artifact
  (``src/segfacet/reference/reference_default.json``), loadable via
  :func:`bundled_default_reference` / :func:`default_artifact_path`
  (``importlib.resources``, mirroring
  ``segfacet.config.default_config_path``/``bundled_default_config``).
* :func:`main` — ``python -m segfacet.reference.artifact [--out JSON]``
...
default::
    ...
    segfacet build-reference --cohort /mnt/verse \\
...
notes, and the deployment-selection mechanism (``segfacet run
--reference-artifact`` / ``reference.artifact_path``).

Scope boundary: this module does not compute delta metrics, add rules, touch
the bounds config, or wire the artifact into ``segfacet run`` (items 046-049);
...
#: deterministic regardless of when ``python -m segfacet.reference.artifact``
#: is actually run.
...
class ReferenceArtifactError(Exception):
    ...
    incompatible ``schema_version`` (mirrors ``segfacet.config.FacetConfigError``)."""
...
def build_reference(
    ...
    *,
    ...
    :class:`~segfacet.reference.schema.ReferenceDistribution`, stamping a
    deterministic :class:`~segfacet.reference.schema.Provenance`.
    ...
    config:
        Defaults to :func:`segfacet.config.bundled_default_config`.
    convention:
        Defaults to :meth:`segfacet.labels.LabelConvention.default`.
    seg_suffix:
...
def build_reference_from_cohort(
    ...
    *,
    ...
    """Build a reference artifact from a resolved :class:`segfacet.datasets.Cohort`
    (Stage 13, item 087) — the dataset-agnostic analogue of
    :func:`build_reference`.

    Ingests the adapter-resolved cohort via
    :func:`segfacet.reference.ingest.ingest_dataset_cohort` (each case's own
...
def load_artifact(path: Union[str, "os.PathLike"]) -> ReferenceDistribution:
    ...
    ReferenceArtifactError
        ...
        for :func:`~segfacet.reference.schema.from_dict`.
    """
    in_path = Path(path)
    ...
    if version != ARTIFACT_SCHEMA_VERSION:
        raise ReferenceArtifactError(
            ...
            f"{in_path}. This version of segfacet supports "
...
def default_artifact_path() -> Path:
    ...
    ``importlib.resources``), mirroring
    :func:`segfacet.config.default_config_path`.
    """
    import importlib.resources as _pkg_resources

    import segfacet.reference as _reference_pkg
...
def build_default_cohort(dest: Union[str, "os.PathLike"]) -> Path:
    ...
    Built from :func:`segfacet.synth.clean_gt.build_clean_spine` under the
    ...
    :func:`segfacet.synth.intensity.paint_clean_scan` (fixed ``seed=0``, the
...
def main(argv: "Optional[Sequence[str]]" = None) -> int:
    """``python -m segfacet.reference.artifact [--out JSON]`` -- regenerate the
    ...
    parser = argparse.ArgumentParser(
        prog="segfacet.reference.artifact",
        description=(
