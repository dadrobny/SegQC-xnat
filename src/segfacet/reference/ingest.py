"""segfacet.reference.ingest — VerSe GT ingestion: cohort loader & feature-
...
(``segfacet.reference.aggregate_reference``) consumes.
...
* It does **not** compute means / percentiles / distributions (that is
  ``segfacet.reference.aggregate.aggregate_reference``, item 043) — it produces
...
* It does **not** change the feature engine — it calls
  ``segfacet.pipeline.extract_feature_record`` unchanged and reads geometry /
...
the Stage 5 corpus convention (``segfacet.synth.corpus.write_corpus``), so a
...
``segfacet.labels.LabelConvention``. Background (value 0) is never a subject
...
# NOT widened: ``segfacet.reference.delta``'s ``_GEOMETRY_FEATURES`` derives
...
def ingest_subject(
    ...
    *,
    ...
    config:
        A :class:`~segfacet.config.HeuristicConfig`, threaded into
        ``extract_feature_record`` unchanged.
    convention:
        The :class:`~segfacet.labels.LabelConvention` used to normalise integer
        labels to canonical names. Defaults to
        :meth:`~segfacet.labels.LabelConvention.default`.
    scan_path:
...
def ingest_dataset_cohort(
    ...
    *,
    ...
    """Ingest a resolved :class:`segfacet.datasets.Cohort` (Stage 13, item 087).
    ...
    already resolved (`segfacet.datasets.resolve`), so nested / varied real datasets
