"""Versioned heuristic-config scaffold for the ``segfacet`` package (item 005).
...
Typical usage::
    ...
    from segfacet.config import default_config, load_config, FacetConfigError
    ...
    try:
        cfg = load_config("/path/to/segfacet_config.yaml")
...
2. **``schema_version`` — strict equality**: any version other than
   ``SUPPORTED_SCHEMA_VERSION`` raises ``FacetConfigError``. Simple and safe
...
3. **Missing-file error wrapped as ``FacetConfigError``**: callers only need to
...
6. **``HeuristicConfig`` is frozen**: immutable after construction, consistent
   with the ``@dataclass(frozen=True)`` style used in ``segfacet.io`` and
   ``segfacet.labels``.
"""
...
class FacetConfigError(Exception):
...
def load_config(path: Union[str, "pathlib.Path"]) -> HeuristicConfig:
    ...
    if version != SUPPORTED_SCHEMA_VERSION:
        raise FacetConfigError(
            ...
            f"This version of segfacet supports schema_version={SUPPORTED_SCHEMA_VERSION!r}."
        )
...
def default_config_path() -> pathlib.Path:
    ...
    pattern ``segfacet.report._load_schema`` already uses for
    ...
    ref = _pkg_resources.files(_segfacet_pkg).joinpath("default_config.yaml")
...
def bundled_default_config() -> HeuristicConfig:
    ...
    is what ``segfacet run`` loads when no ``--config`` flag is given (item 035).
