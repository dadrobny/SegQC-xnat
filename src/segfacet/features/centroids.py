...
* **level_name** — anatomical vertebra name (e.g. ``"C1"``, ``"L3"``, ``"S"``)
  looked up from a :class:`segfacet.labels.LabelConvention`.  Falls back to
  :data:`segfacet.labels.UNKNOWN` for unmapped integer labels; never empty.
...
class LabelCentroid:
    ...
    level_name:
        ...
        (e.g. ``"C1"``, ``"T8"``, ``"S"``), or :data:`~segfacet.labels.UNKNOWN`
...
def compute_centroid(
    ...
    **deterministic**: identical inputs always produce identical outputs.
    ...
    convention:
        Optional :class:`~segfacet.labels.LabelConvention` used to resolve the
...
class CentroidFeatures:
    ...
    level_name:
        ...
        :data:`~segfacet.labels.UNKNOWN`; never empty.
    is_atlas_axis:
...
def compute_edt_centroids(
    ...
    *,
    ...
    convention:
        Optional :class:`~segfacet.labels.LabelConvention`; defaults to
