...
This is a thin wrapper over :func:`segfacet.features.components.compute_components`,
...
def compute_fragmentation_index(
    ...
    *,
    ...
    :func:`segfacet.features.components.compute_components`; it does not duplicate
    ...
    config:
        A :class:`~segfacet.config.HeuristicConfig` instance (forwarded to
        :func:`~segfacet.features.components.compute_components`).
    backend:
        Optional :class:`~segfacet.backend.Backend` handle, forwarded unchanged
        to :func:`~segfacet.features.components.compute_components`. When
        ``None`` (the default), ``compute_components`` auto-resolves it via
        :func:`segfacet.backend.get_backend`.
    ...
    """
    from segfacet.features.components import compute_components  # lazy import
