...
(:mod:`segfacet.features.intensity`). PyRadiomics (PyPI package ``pyradiomics``,
...
scope so that :mod:`segfacet.features.radiomics` itself always imports cleanly,
...
``first_order``
    The item-059 :class:`~segfacet.features.intensity.LabelIntensity` block.
``extended``
...
# Guarded optional import: segfacet.features.radiomics must import cleanly even
...
class LabelRadiomics:
    ...
    ----------
    first_order:
        The item-059 :class:`~segfacet.features.intensity.LabelIntensity`
