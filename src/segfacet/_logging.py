"""Structured logging helpers for the ``segfacet`` package (item 005).

All ``segfacet.*`` loggers are children of the top-level ``"segfacet"`` logger, so
calling :func:`setup_logging` once is enough to configure the whole hierarchy.

Usage (in ``segfacet`` modules)::

    import logging
    logger = logging.getLogger(__name__)   # e.g. "segfacet.config", "segfacet.io"
    logger.info("something happened")

Callers that want structured output::

    from segfacet._logging import setup_logging
    setup_logging("DEBUG", json_format=True)

Design decisions (item 005)
----------------------------
- **Module name ``_logging`` not ``logging``**: naming the submodule
  ``segfacet.logging`` would shadow the stdlib ``logging`` module from inside
...
# The package-wide root logger. All ``segfacet.*`` child loggers propagate here.
_PACKAGE_LOGGER_NAME = "segfacet"
...
class JsonFormatter(logging.Formatter):
    ...
    ``logger``
        The logger name, e.g. ``"segfacet.config"``.
    ``message``
...
def setup_logging(
    ...
    *,
    ...
    """Configure the ``"segfacet"`` logger hierarchy.
    ...
    (idempotent): existing handlers on the ``"segfacet"`` logger are removed before
    ...
    ----------
    level:
        Log level for the ``"segfacet"`` logger. Accepts the standard stdlib names
