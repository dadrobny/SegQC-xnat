...
existing Stage 1 QC verdict model (:mod:`segfacet.verdict`: ``Severity``
...
def finding_to_reason(finding: Finding) -> Reason:
    """Map a single :class:`Finding` to a :class:`segfacet.verdict.Reason`.
...
def aggregate_verdict(
    ...
    *,
    ...
    config:
        A :class:`segfacet.config.HeuristicConfig` (or compatible object exposing
...
class CaseResult:
    ...
    ----------
    verdict:
        The aggregated :class:`segfacet.verdict.Verdict`.
    findings:
...
def build_case_result(
    ...
    *,
    ...
    findings:
        The findings to aggregate, in order. Not mutated.
    config:
        A :class:`segfacet.config.HeuristicConfig` (or compatible) providing the
        severity policy.
    base_reasons:
