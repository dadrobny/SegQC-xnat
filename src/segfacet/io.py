"""NIfTI input/output for ``segfacet`` — the in-memory volume model.
...
:class:`FacetInputError` exceptions and exposes only a *raw* label inventory
...
class FacetInputError(Exception):
...
def load_volume(path: PathLike, *, integer_labels: bool = False) -> Volume:
    ...
    if not os.path.exists(path_str):
        raise FacetInputError(f'Input file does not exist: "{path_str}"')
...
def load_case(scan_path: PathLike, seg_path: PathLike) -> Case:
    ...
    :class:`FacetInputError` naming the mismatch is raised. A raw label
