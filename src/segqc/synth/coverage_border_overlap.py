...
The second Stage 5 operator family: three seeded :class:`~segqc.synth.
...
(:func:`segqc.synth.clean_gt.build_clean_spine`), each returning a
well-formed :class:`~segqc.synth.perturbation.Expectation` naming the induced
...
* :class:`RemoveLevelPerturbation` (``"remove_level"``) -- deletes an
  ...
  that :class:`~segqc.heuristics.coverage.CoverageRule` (item 029) detects as
...
* :class:`CropAtBorderPerturbation` (``"crop_at_border"``) -- translates a
  ...
  :class:`~segqc.heuristics.border.BorderRule` (item 031) fires a
...
* :class:`ForceOverlapPerturbation` (``"force_overlap"``) -- shifts an
  ...
  two-channel mask stack fed to :func:`segqc.features.overlap.detect_overlaps`
  / :class:`~segqc.heuristics.overlap.OverlapRule` directly (§6 mode 8; see
...
# axis 2 = anterior-posterior (matches segqc.features.geometry / clean_gt).
_FACE_AXIS_SIDE = {
    "inferior": (0, "low"),
    "superior": (0, "high"),
    "left": (1, "low"),
...
class RemoveLevelPerturbation(Perturbation):
    ...
    :class:`~segqc.heuristics.coverage.CoverageRule` detects via
...
class CropAtBorderPerturbation(Perturbation):
    ...
    touches the face (driving :class:`~segqc.heuristics.border.BorderRule`,
...
class ForceOverlapPerturbation(Perturbation):
    ...
    :func:`segqc.features.overlap.detect_overlaps` /
    :class:`~segqc.heuristics.overlap.OverlapRule` directly (§6 mode 8).
