...
Turns one manifest case dict (from :func:`segqc.synth.corpus.load_manifest`)
...
committed seg fixture via the Stage 0 loader (:func:`segqc.io.load_case`),
runs the real pipeline (:func:`segqc.pipeline.run_qc`), and exposes pure
...
* ``detection == "reconstructed_record"`` -- the failure mode is structurally
  ...
  the designated rule (:class:`~segqc.heuristics.mislabel.MislabelRule` /
  :class:`~segqc.heuristics.overlap.OverlapRule`).
...
# --------------------------------------------------------------------------- #
# Loading -- the Stage 0 path, same as ``segqc run``
# --------------------------------------------------------------------------- #


def loaded_seg_image(case: dict, corpus_dir: Path = CORPUS_DIR) -> "nib.Nifti1Image":
    """Load *case*'s committed seg fixture via :func:`segqc.io.load_case` and
