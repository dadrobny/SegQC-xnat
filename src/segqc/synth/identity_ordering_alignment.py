...
:class:`~segqc.synth.perturbation.Perturbation` subclasses that inject
label-identity / ordering / spatial-alignment failures onto the item-036
clean-GT positive control (:func:`segqc.synth.clean_gt.build_clean_spine`),
each returning a well-formed :class:`~segqc.synth.perturbation.Expectation`
...
* :class:`DisplacePerturbation` (``"displace"``) -- translates a target
  ...
  :class:`~segqc.heuristics.mislabel.MislabelRule` (item 033, Detector A,
...
* :class:`SequenceBreakPerturbation` (``"sequence_break"``) -- relabels the
  ...
  sequence that :class:`~segqc.heuristics.sequence.SequenceRule` (item 030,
...
# segqc.labels.DEFAULT_LABEL_MAP / CANONICAL_ORDER).
...
class SequenceBreakPerturbation(Perturbation):
    ...
    **tail**, the max present label) to ``new_label`` (default 28 == T13),
    ...
    that :class:`~segqc.heuristics.sequence.SequenceRule` catches directly
