...
that the real Stage 4 pipeline (:func:`segqc.pipeline.run_qc`, under the
...
Axis convention (matching :mod:`segqc.features.geometry`): image axis 0 is
...
vertebrae T13/L6 in :data:`segqc.labels.CANONICAL_ORDER` and would (correctly)
trip the missing-interior-level coverage check; :func:`build_clean_spine`
raises :class:`~segqc.io.SegQCInputError` for such a span rather than silently
...
class CleanSpine:
    ...
    scan_img:
        ...
        ``seg_img``), so the pair can be driven through ``segqc run`` (item
        035/010) end-to-end.
    seg_img:
...
def build_clean_spine(
    *,
    ...
    convention:
        The :class:`~segqc.labels.LabelConvention` to resolve level names to
