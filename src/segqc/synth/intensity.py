...
Extends the Stage-5 synthetic generator (:mod:`segqc.synth.clean_gt`,
:mod:`segqc.synth.corpus`) with the tool's first use of scan **voxel
...
And the committed-corpus surface (mirrors :mod:`segqc.synth.corpus`, item
040): :func:`build_intensity_corpus`, :func:`write_intensity_corpus`,
:func:`load_intensity_manifest`, and a ``python -m segqc.synth.intensity
...
from segqc.synth.perturbation import seeded_rng
...
def paint_clean_scan(
    ...
    from :func:`~segqc.synth.perturbation.seeded_rng` (never the global
...
def _save_deterministic(img: nib.Nifti1Image, path: Path) -> None:
    ...
    ``segqc.synth.corpus._save_deterministic`` (nibabel 5.3.3's gzip writer
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    """``python -m segqc.synth.intensity [--out DIR]`` -- regenerate the
