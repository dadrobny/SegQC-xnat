...
This is the first feature family in ``segqc`` to read scan **intensities**
...
-----------------------------
Unlike :func:`segqc.features.geometry.compute_label_geometry`, which raises
...
``segqc.io.load_case`` (``rtol=1e-5, atol=1e-4``); shape must match exactly.
...
# Affine-tolerance for the grid-alignment guard, matching segqc.io.load_case.
_AFFINE_ATOL = 1e-4
_AFFINE_RTOL = 1e-5
...
def _check_alignment(scan_img: nib.Nifti1Image, seg_img: nib.Nifti1Image) -> None:
    ...
    Mirrors ``segqc.io.load_case``'s shape/affine checks and message style.
    """
