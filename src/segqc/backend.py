...
SegQC-xnat is **CPU-only by default and zero-required-GPU-dependency**: the
...
1. An explicit ``override`` argument (e.g. the future ``segqc run --backend``
   flag from item 075) -- used whenever it is non-``None`` and non-empty after
   stripping whitespace.
2. The ``SEGQC_BACKEND`` environment variable -- used when ``override`` is
...
is unavailable raises :class:`SegQCBackendError` -- a clean, actionable
...
``segqc.features.radiomics``'s module-scope-cached ``pyradiomics_available``
...
- Add a CLI surface (``segqc run --backend ...``) -- that is item 075's job;
  this module stops at the ``SEGQC_BACKEND`` env var and the programmatic
...
class SegQCBackendError(Exception):
    ...
    - GPU explicitly requested (override or ``SEGQC_BACKEND``) while ``cupy``
...
def resolve_backend_choice(override: Optional[str] = None) -> str:
    ...
    stripping whitespace) > the ``SEGQC_BACKEND`` environment variable (when
    non-empty after stripping) > the default token ``"auto"``. Tokens are
    matched case-insensitively after ``.strip().lower()``.

    Raises:
        SegQCBackendError: if the effective token is not one of ``cpu``,
    ...
    if token not in _VALID_TOKENS:
        raise SegQCBackendError(
            f"Invalid SegQC backend {token!r}. "
    ...
    if resolved == BACKEND_GPU and not cupy_available():
        raise SegQCBackendError(
            ...
            "(e.g. `pip install segqc[gpu]` plus a CUDA-matched cupy-cudaXXx "
...
def backend_name(override: Optional[str] = None) -> str:
    ...
    :class:`SegQCBackendError` for invalid tokens or an unavailable forced-GPU
    request.
    """
...
def get_backend(override: Optional[str] = None) -> Backend:
    ...
    Raises:
        SegQCBackendError: propagated from :func:`resolve_backend_choice`.
    """
