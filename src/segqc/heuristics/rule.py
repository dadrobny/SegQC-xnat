...
class Rule(abc.ABC):
    ...
    rule_id:
        ...
        :class:`~segqc.heuristics.Finding` the rule emits.
    ...
    evaluate(record, config) -> list[Finding]:
        ...
        zero or more :class:`~segqc.heuristics.Finding` objects.  An empty
    ...
    def evaluate(self, record, config) -> List[Finding]:
        ...
        config:
            A :class:`~segqc.config.HeuristicConfig` instance.  Use
