...
from segqc.heuristics.rule import Rule, iter_rules
...
def run_rules(
    ...
    record:
        ...
        produced by ``segqc.feature_report.build_features_block``).  The
        runner never mutates this mapping.
    config:
        A :class:`~segqc.config.HeuristicConfig` instance.  Each rule's
        enabled state is queried via ``config.rule_enabled(rule.rule_id)``.
    rules:
        An optional iterable of :class:`~segqc.heuristics.Rule` instances to
        run.  When ``None`` (the default), all rules currently in the registry
        are used via :func:`~segqc.heuristics.rule.iter_rules` (sorted
