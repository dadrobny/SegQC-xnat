...
A ``Finding`` is the atomic output unit of a :class:`segqc.heuristics.Rule`.
...
- Reuses ``segqc.verdict.Severity`` (PASS < FLAG < FAIL) so findings can flow
...
class Finding:
    ...
    severity:
        The severity level of this finding.  Reuses ``segqc.verdict.Severity``
