...
``segqc.pipeline.extract_feature_record(seg_img, config)`` -- the whole
...
``SEGQC_BACKEND`` environment variable (item 071) around each timed block and
invoking the **unmodified** ``extract_feature_record``: item 072's ported
feature functions resolve ``backend=None -> segqc.backend.get_backend()`` at
...
``run_benchmark`` / the ``python -m segqc.benchmark`` script entry point and
...
``tokens``/``backends`` when ``segqc.backend.cupy_available()`` is ``True``.
This module never imports ``cupy`` at module scope -- it is imported (if at
all) transitively through ``segqc.backend.get_backend`` only when the GPU
...
def _time_backend(token, seg_img, config, *, iterations, warmup):
    ...
    calls. Restores the prior ``SEGQC_BACKEND`` value on exit."""
    env_var = segqc.backend.ENV_VAR
...
def run_benchmark(*, case_id=None, iterations=DEFAULT_ITERATIONS, warmup=DEFAULT_WARMUP):
    ...
    the ``gpu`` backend (only when ``segqc.backend.cupy_available()`` is
    ...
    tokens = ["cpu"]
    if segqc.backend.cupy_available():
        tokens.append("gpu")
    ...
    return {
        ...
        "iterations": iterations,
        "warmup": warmup,
        "cupy_available": segqc.backend.cupy_available(),
...
def main(argv=None) -> int:
    """``python -m segqc.benchmark`` script entry point.

    Runs :func:`run_benchmark` and writes the resulting report as JSON to
    ``--out``, printing a one-line human-readable summary.
    """
    parser = argparse.ArgumentParser(
        prog="segqc.benchmark",
        description=(
            "Benchmark segqc.pipeline.extract_feature_record's wall-clock "
    ...
    print(
        f"segqc.benchmark: wrote report to {args.out} "
