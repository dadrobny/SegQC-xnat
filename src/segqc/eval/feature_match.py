...
``segqc.pipeline.extract_feature_record`` / ``segqc.feature_report.build_features_block``)
...
:data:`segqc.labels.CANONICAL_ORDER` / :data:`segqc.labels.UNKNOWN` supply
...
This module is unrelated to item 050's :mod:`segqc.eval.overlap` (voxel-level
...
from segqc.labels import CANONICAL_ORDER, UNKNOWN
...
class LabelFeatureDivergence:
    ...
    name:
        ...
        ``level_name`` for an unmatched label (``segqc.labels.UNKNOWN`` if
...
class FeatureMatchResult:
    ...
    per_label:
        ...
        :data:`segqc.labels.CANONICAL_ORDER` for recognised names then by
...
def compute_feature_match(
    ...
    candidate, gt:
        ...
        ``segqc.pipeline.extract_feature_record`` /
        ``segqc.feature_report.build_features_block`` -- each carrying a
