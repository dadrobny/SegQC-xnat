...
``segqc.synth.perturbation.Expectation.to_dict()`` / a ``tests/corpus``
manifest-case entry, or a hand-built dict for a human-provided expectation)
and an **actual** side (the pipeline's ``segqc.aggregate.CaseResult``, whose
...
severity, ``positive_severity`` (default :data:`segqc.verdict.Severity.FLAG`):
...
``segqc.aggregate.CaseResult`` is the reference type.
"""
...
def _extract_expected(
    ...
    _severity_of(expected_verdict)  # validates; raises SegQCInputError if unknown
