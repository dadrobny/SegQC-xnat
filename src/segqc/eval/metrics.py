...
Given an already-computed :class:`~segqc.eval.harness.CohortEvaluation` (a
tuple of :class:`~segqc.eval.harness.CaseEvaluation` records, each bundling
...
decoupled from ``segqc.synth`` -- the §6 failure-mode integer keys are
...
class CohortMetrics:
    ...
    def to_dict(self) -> dict:
        ...
        Mirrors ``segqc.eval.harness``'s ``_tuples_to_lists`` approach: every
...
# --------------------------------------------------------------------------- #
# JSON-shape helper (mirrors segqc.eval.harness._tuples_to_lists)
# --------------------------------------------------------------------------- #
...
def compute_cohort_metrics(
    ...
    *,
    ...
    ----------
    cohort:
        A :class:`~segqc.eval.harness.CohortEvaluation`, or any object
        duck-typed on ``.cases`` (an iterable of
        :class:`~segqc.eval.harness.CaseEvaluation`-shaped records). Read
        only, never mutated.
    correlation_method:
