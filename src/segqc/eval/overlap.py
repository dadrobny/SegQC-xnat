...
anatomical name it maps to. A single :class:`~segqc.labels.LabelConvention`
(default :meth:`~segqc.labels.LabelConvention.default`) is used only to attach
...
This module is unrelated to :mod:`segqc.features.overlap` (item 015), which
...
class LabelOverlap:
    ...
    name:
        Anatomical display name from the :class:`~segqc.labels.LabelConvention`
        used (``segqc.labels.UNKNOWN`` if unmapped). Naming/ordering only —
...
class OverlapResult:
    ...
    per_label:
        ...
        :data:`segqc.labels.CANONICAL_ORDER` for recognised labels then by
...
def _order_key(value: int, name: str) -> Tuple[int, int]:
    ...
    Mirrors ``segqc.labels._order_key`` but keys on the integer ``value``
...
def compute_overlap(
    ...
    *,
    ...
    convention:
        The :class:`~segqc.labels.LabelConvention` used to name and order
        entries. Defaults to :meth:`~segqc.labels.LabelConvention.default`.
