...
Renders item 054's cohort-level :class:`~segqc.eval.metrics.CohortMetrics`
and item 055's optional :class:`~segqc.eval.calibrate.CalibrationResult` into
...
:func:`segqc.config.load_config`).

This is the Stage-7 analogue of the Stage-1 per-case report (see
``segqc.report``/items 009 and ``segqc.human_report``/item 010): a bundled
...
``EvaluationProvenance``
    ...
    ``build_date``, optional ``reference_schema_version``/``segqc_version``).
...
Dependencies: :mod:`segqc.eval.metrics` (item 054), :mod:`segqc.eval.calibrate`
(item 055), :mod:`segqc.config` (items 005/035), ``jsonschema``, ``PyYAML``.
"""
...
if TYPE_CHECKING:  # pragma: no cover - typing only
    ...
    from segqc.eval.calibrate import CalibrationResult, ThresholdAxis
    from segqc.eval.metrics import CohortMetrics
...
def _load_eval_schema() -> dict:
    ...
    (mirrors ``segqc.report._load_schema``)."""
    import segqc.eval as _eval_pkg  # local import to avoid circular deps at module level
...
class EvaluationProvenance:
    ...
    segqc_version:
        Optional segqc package version string.
    """
    ...
    segqc_version: Optional[str] = None
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "segqc_version": self.segqc_version,
        }
...
def _calibration_block(calibration: "CalibrationResult") -> dict:
    """Reduce a :class:`~segqc.eval.calibrate.CalibrationResult` to the
...
def build_evaluation_report(
    ...
    *,
    ...
    ----------
    metrics:
        The :class:`~segqc.eval.metrics.CohortMetrics` to embed verbatim
        (via ``metrics.to_dict()``) under ``"metrics"``.
    provenance:
        The :class:`EvaluationProvenance` to embed (via ``to_dict()``) under
        ``"provenance"``.
    calibration:
        Optional :class:`~segqc.eval.calibrate.CalibrationResult`. When
...
def render_evaluation_report(
    ...
    *,
    ...
    ``segqc.human_report.render_human_report``): every value is formatted
    ...
    ----------
    metrics:
        The :class:`~segqc.eval.metrics.CohortMetrics` to render.
    provenance:
    ...
    title = f"SegQC Evaluation Report -- {provenance.cohort_id}"
    lines.append(title)
...
def record_calibrated_config(
    ...
    calibration_result:
        The :class:`~segqc.eval.calibrate.CalibrationResult` whose ``best``
        candidate's assignment is applied.
    axes:
        The :class:`~segqc.eval.calibrate.ThresholdAxis` sequence whose
