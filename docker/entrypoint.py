"""Container entry script — XNAT mounted-directory inputs -> ``segqc run`` (item 068).

This script is the process the container image actually invokes for the
`segqc-xnat` XNAT Container Service command (item 067's ``command.json``). It
translates XNAT's mounted-directory input/output convention into a single
``segqc run`` invocation and leaves the two report files where XNAT collects
...
- ``--scan-dir`` / ``--seg-dir`` (required): resolve to the single NIfTI file
  ...
  in the directory, mapped to ``segqc run --scan`` / ``--seg``.
- ``--out-dir`` (required): passed verbatim as ``segqc run --out``. ``segqc
  run`` itself creates the directory and writes ``segqc_report.json`` /
  ``segqc_report.txt`` there.
- ``--config-dir`` (optional): resolves the single ``*.yaml``/``*.yml`` file,
  if any, mapped to ``segqc run --config``. Absent/empty is a no-op (the
  bundled default config is used).
- ``--reference-dir`` (optional): resolves the single ``*.json`` file, if
  any, mapped to ``segqc run --reference --reference-artifact``. Absent/empty
  is a no-op.
- ``--reference`` / ``--intensity`` (``store_true`` toggles): forwarded
  verbatim to ``segqc run --reference`` / ``--intensity``. ``--reference`` is
...
pipeline is invoked in-process via ``segqc.cli.main(argv)``, not by shelling
out to the ``segqc`` console script -- this reuses ``segqc.cli``'s own
...
*before* ``segqc.cli.main`` is ever called, so a broken mount never leaves a
...
# segqc.io.load_volume documents.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "inputs to a single `segqc run` invocation."
        ),
    )
    parser.add_argument(
        "--scan-dir",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable reference mode (forwarded to `segqc run --reference`).",
    )
    parser.add_argument(
        "--intensity",
        action="store_true",
        default=False,
        help="Enable intensity mode (forwarded to `segqc run --intensity`).",
    )
    return parser
...
def build_run_argv(
    ...
    """Pure, side-effect-free assembler for the ``segqc run`` argv.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ``segqc run`` argv, and invokes ``segqc.cli.main`` -- resolved at call
    time (``import segqc.cli`` then ``segqc.cli.main(...)``) so tests that
    monkeypatch ``segqc.cli.main`` are observed. Any input-resolution error
    ...
    *before* ``segqc.cli.main`` is ever called for a resolution error, so a
    ...
    try:
        import segqc.cli  # noqa: PLC0415

        return segqc.cli.main(run_argv)
